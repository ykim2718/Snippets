#!/bin/ksh
#951124,(c)shik,CSP5HS,0.4um CMOS Logic Process,C5HS Mask
#960309,(c)shik,CSP5HS,0.4um CMOS Logic Process,C5HB Mask
#960329,(c)shik,CSP5HB,0.5um CMOS Logic Process,C5HB Mask

#########1#########2#########3#########4#########5#########6#########7#########8
name="c5v-16-plug,PAD"
path=".\/O"
file[0]="n1"
range=90
lgType=1
...
sed -e "s/XXX_NAME/$name/g"   -e "s/XXX_FILE/${file[0]}/g" \
    -e "s/XXX_RANGE/$range/g" -e "s/XXX_PATH/$path/g"  core-Gr > core-Gr.p
\rm -f core-Gr core-[0-9][0-9]*
grx core-Gr.p -geometry 600x800+0+0 -s
...
%gBeg_
 ...
 _defn "WIDTH"    "100"
 .glob 
       gtxt txt="CSP5HB5V,XXX_NAME,W=WIDTHum" t.width=10 t.height=20
       gout psf portrait name="XXX_NAME"
! (c)shik : core-01
 .plot 
       attr moment ylin
       data name="XXX_PATH/plug-tr-w100-vth.XXX_FILE.plt" switch
...
! (c)shik : core-03
 .plot
       attr moment ylin
       data name="XXX_PATH/plug-tr-w100-swing.XXX_FILE.plt" switch
       slct yno=(*) xno=1 XB ylb=70 yub=110 range=XXX_RANGE
...
! (c)shik : core-05
 .plot 
       attr moment ylin
       data name="XXX_PATH/plug-tr-w100-ids50.XXX_FILE.plt" switch
       slct yno=(*) xno=1 XB ylb=300 yub=700 range=XXX_RANGE
       +    lg.type=XXX_LGTYPE lg.size=1 FILL
       ufun seqn=0 fn=6 pgm="z=abs(z)*1e6/WIDTH" ncl=2 nch=10000
...
! (c)shik : core-06
 .plot 
       ...
       data name="XXX_PATH/plug-tr-w100-ids50.XXX_FILE.plt" switch
       slct yno=(*) xno=1 range=XXX_RANGE lg.type=XXX_LGTYPE lg.size=1 FILL
       ufun seqn=0 fn=6 pgm="z=abs(z)*1e6/WIDTH" ncl=2 nch=200
! (c)shik : core-07
 .plot 
       attr moment ylog
       data name="XXX_PATH/plug-tr-w100-ids0.XXX_FILE.plt" switch
       slct yno=(*) xno=1 XB ylb=1e-15 yub=1e-6 range=XXX_RANGE
       +    lg.type=XXX_LGTYPE lg.size=1 FILL
       ufun seqn=0 fn=6 pgm="z=abs(z)/WIDTH" ncl=2 nch=200
...
! (c)shik : core-09
 .plot 
       attr moment ylin
       data name="XXX_PATH/plug-tr-w100-bv10nA.XXX_FILE.plt" switch
       slct yno=(*) xno=1 XB ylb=0 yub=13 range=XXX_RANGE
       +    lg.type=XXX_LGTYPE lg.size=1 FILL
       ufun seqn=0 fn=6 pgm="z=abs(z)"
...
! (c)shik : core-11
 .plot 
       attr moment ylog
       data name="XXX_PATH/plug-tr-w100-isub50.XXX_FILE.plt" switch
       slct yno=(*) xno=1 XB ylb=1e-9 yub=1e-3 range=XXX_RANGE
       +    lg.type=XXX_LGTYPE lg.size=1 FILL
       ufun seqn=0 fn=6 pgm="z=abs(z)/WIDTH" ncl=2 nch=200
       axis type=y,0 label=t lb.f="%g" TEXT3 LOGA
       +    type=y,1 axis=upper
