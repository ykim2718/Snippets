...
void CColorBtn::DrawItem(LPDRAWITEMSTRUCT lpd)
{
   ...
   colorBrush.CreateSolidBrush(nCurrentColor);
   ...
   if (!(lpd->itemState & ODS_DISABLED)) {
      ...
      DC.SelectObject(dkgray);
      DC.SelectObject(&colorBrush);
      DC.Rectangle(left+4,top+3,right-4,bottom-3);
   }
...
void CColorBtn::OnClicked() 
{
   // When the button is clicked, show the dialog.
	...
	if (dlg.DoModal() == IDOK) {
      nCurrentColor = CColorBtnDlg::colors[dlg.colorIndex];
      InvalidateRect(NULL);
      CWnd *parent=GetParent();
      parent->SendMessage(WM_COLORBTNMESSAGE,WM_COLORBTNMESSAGE,(LPARAM)GetDlgCtrlID());
   }
}
...
{
   return
   (AfxGetApp()->WriteProfileBinary("ColorData","ColorTable",(LPBYTE)CColorBtnDlg::colors,sizeof(COLORREF)*20) &&
...
BOOL CColorBtn::Load()
{
   ...
   if (data) {
      ...
      delete (void*)data;//free((void *)data);      
