...
const ListDemoActionCreator = {
    getLoadingAction(show) {
        return { type: C.IS_LOADING, isLaoding: show }
    },
        ...
        return (dispatch, getState) => {
            ...
            setTimeout( () => {
                if (missingIndex >= 0) {
                    ...
                    dispatch(this.getLoadingAction(333)
                    console.log("going to show alert dialog sooooooooon", missingIndex, virtual_db.length)
                }
            }, 10000) // 1 sec delay to mimic asynchronous operation
        }
    },
    delListItem(id) {
