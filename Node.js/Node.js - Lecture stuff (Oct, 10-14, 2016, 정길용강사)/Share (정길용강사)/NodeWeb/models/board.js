...
module.exports = {
		...
		show: function(no, cb){
			var b3 = db.board.findOneAndUpdate({_id: parseInt(no)}
										, {$inc: {view: 1}}
										, function(err, result){
											cb(result.value);
										});			
		...
		create: function(board, cb){
			board.regdate = require('date-format').asString('MM.dd hh:mm', new Date());
			board.view = 0;
