...
db.board.find({writer: '김철수'}, {writer: 1, _id: 0});
db.board.find({}, {writer: 1, _id: 0});
db.board.find().sort({_id: -1}).limit(1);
// 모든 게시물을 _id의 내림차순으로 조회(출력 컬럼은 번호, 제목, 글쓴이, 조회수, 작성일)
...
db.board.update({_id: 1}, {title: '수정한 제목'});
db.board.update({_id: 1}, {$set: {title: '수정한 제목'}});
