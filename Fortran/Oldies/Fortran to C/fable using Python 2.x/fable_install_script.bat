...
if not exist fable_sources\TAG goto find_python
echo.
echo Build tag:
type fable_sources\TAG
...
:have_python
echo.
echo Configuring fable build directory
call "%python%" ..\fable_sources\libtbx\configure.py fable
set el=%errorlevel%
cd ..
if not %el% == 0 goto end
call fable_build\setpaths_all.bat

if not exist fable_install_finalize.bat goto run_tests
echo Running final setup script fable_install_finalize.bat
call fable_install_finalize.bat
