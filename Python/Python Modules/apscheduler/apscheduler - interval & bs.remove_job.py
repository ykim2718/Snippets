...
def my_function(*args, _count=[0], **kwargs):
    ...
    end_time = kwargs['end_time'].split(':')
    start_time = now.normalize().replace(hour=int(start_time[0]), minute=int(start_time[1]), second=int(start_time[2]))
    end_time = now.normalize().replace(hour=int(end_time[0]), minute=int(end_time[1]), second=int(end_time[2]))
    _count[0] += 1
    if start_time <= now <= end_time:
        print(f"\t[{_count[0]}] {start_time=}, {end_time=}")
    elif end_time < now:
...
end_time = (now + pd.Timedelta(seconds=10)).strftime("%H:%M:%S")
# bs.add_job(my_function, 'interval', minutes=1, start_date=start_date, end_date=end_date, args=['args here'])
job = bs.add_job(
    my_function, 'interval', seconds=2, start_date=start_date, end_date=end_date,
    args=['args here'], id=job_id, kwargs=dict(bs=bs, job_id=job_id, start_time=start_time, end_time=end_time))
print(f"{bs.state=}")
