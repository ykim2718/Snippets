...
Description: Allocation debugger
    Creator: Kim, Yongshik
       Date: 43320417 (990417)
   Revision: Kim, Yongshik (copyRight)
     Update: 990417,0501
             4334 0220-0306
Last Update: 20160328-0404
...
{
   int32u nIndex,nSize,nBrokenObject;
   char *pAddr,*pAdjusted,*pFence;

   nBrokenObject=0;
   for (nIndex=0;nIndex<myAllocStackTotal;nIndex++) {
      pFence=pAdjusted=pAddr=myAllocStack[nIndex];
      pAdjusted-=sizeof(int32u);
      nSize=*((int32u *)pAdjusted);
      pFence=pAddr+nSize;
   ...
   if (nBrokenObject) return (int)nBrokenObject;
   return (int)myAllocStackTotal;
   /* The best is zero-return. */
}

void *myAlloc( nSize ) /* sizeof(int)+sizeof(int32u)+pAddr+FENCEWORD(unsigned int) */
   int32u nSize;
{
   void *pAddr;
   char *pAdjusted;
   ...
   lsize=sizeof(int32u)+nSize+sizeof(unsigned int);
   lsize=sizeof(int)+sizeof(int32u)+nSize+sizeof(unsigned int);
   if (!(pAddr=(void *)malloc(lsize))) {
      sprintf(
         qBuffer,
         "Error, \"%zu\" bytes allocation ... in myAlloc() !\n",
         lsize);
      errExit(qBuffer);
   }
   ...
   *((int32u *)pAdjusted)=nSize;
   pAddr=(void *)(pAdjusted+=sizeof(int32u));
   pAdjusted+=nSize;
   ...
   int32u nSize;
{
   ...
   nOldSize=*(int32u*)((char*)pAddr-sizeof(int32u)); /* 2001.0306 */
   ...
   void *pAddr;
{
   ...
   nSize=*((int32u *)pAdjusted);
   pFence+=nSize;
   pAdjusted-=sizeof(int); /* DEBUG : *(int32u*)((char*)(myAllocStack[0])-8) */
   if (*((unsigned int *)pFence)!=FENCEWORD) {
      sprintf(
         qBuffer,
