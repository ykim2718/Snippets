   ...
   int *nLineStart,*nLineEnd;
{
   ...
   while (pStream[0]) {
      ...
      nSize=pTemp ? (int)(pTemp-pStream):(int)strlen(pStream);
      pNode=(DLN*)malloc(sizeof(DLN));
      if (nSize) {
   ...
   char **ppStream;
{
   ...
   for (pNode=pList->head;pNode!=pList->tail->next;pNode=pNode->next) {
      ...
      pStream+=(int)strlen(pNode->tok);
      pStream[0]=' ';
      pStream++;
   }
   nLength=(int)strlen(*ppStream);
   return nLength;
}
   ...
   const char *qFileName;
{
   ...
   pNode=pList->head;
   while (pNode) {
      nIndex=(int)strlen(pNode->tok)+1+sizeof(int);
