/******************************************************************************
       ...
       File: yFile.c
Description: Modules associated with files
    Creator: Kim, Yongshik (copyRight)
       Date: 43230724
   Revision: Kim, Yongshik
     Update: 4323 0724-1231,
             4324 0101-042,
             4327 1002,
             4328 0804,1031,
             4329 0320,0321,0418,0827,0913,
             4330 0423,0831,
             4331 0930,
             4332 0505,0708,
             4333 0222,0403-0529,0608,0622,0811-0828,1124,
             4334 0219,0515,
             4336 0211-0216,0308,0529
...
#  define DLL_MAX_NO 2e4
   ...
   pTo=strchr(pFrom,'\n');
   while (pTo && *pTo) {
      pAddr=strmid((const char*)pFrom,0,(int)(pTo-pFrom));
      pFrom=pTo+1;
      strtrim(pAddr);
      if (isnumstr(pAddr)) {
         if (bBlockFound==-1) {
            if (nBlockIndex && vBlockIndex) {
               nBlockCount++;
               bBlockFound=
                  VIsInIntVector(nBlockCount,vBlockIndex,0,nBlockIndex-1);
            } else
               bBlockFound=TRUE;
         }
         if (bBlockFound==TRUE) {
   ...
   const char *qRefPath,*qPath; /* must be full path and folder only */
{
   ...
   strtrim(pRefPath);
   strtrim(pPath);
   nRefPath=(int)strlen(pRefPath);
   nPath=(int)strlen(pPath);
   ...
   while (nCount-->0) {
      ...
      nIndex+=(int)strlen(qDoubleDot);
      strcpy(pRefPath+nIndex,qFolder);
      nIndex+=(int)strlen(qFolder);
   }
...
{
   fprintf(stderr,"\n");
   fprintf(stderr,"Y, Kim (copyRight) %s\n",_VERSION_);
   fprintf(stderr,"\n");
   exit(1); /* in sysExit() */
}
